...
import { ManutencaoViatura, Viatura } from "../../@types/types";
...
export function DrawerManutencao({ viatura }: { viatura: Viatura }) {
    const { isOpen, onOpen, onClose } = useDisclosure()
    const btnRef = React.useRef()
    return (
        <>
            <Button mr={2} boxShadow="buttonShadow" ref={btnRef} leftIcon={<GiTowTruck fontSize="20" />} bg={"gray.700"} _hover={{ bg: "gray.800" }} size='sm' color={"white"} onClick={onOpen}>
                Relatório
            </Button>
            <Drawer
                isOpen={isOpen}
            ...
            >
                ...
                <DrawerContent bg='gray.990' shadow="buttonShadow">
                    ...
                    <DrawerHeader borderBottom="1px solid" borderColor="green.600">
                        ...
                        <Text color={"green.600"}>{viatura?.tipo}</Text>
                    </DrawerHeader>
                    <DrawerBody>
                        {viatura?.ManutencaoViatura?.length == 0 ? <NotData textoComponent={"Nenhuma manutenção registrada"} /> : (
                            <Grid gridTemplateColumns={["1fr 1fr 1fr"]} gap={2}>
                                {viatura?.ManutencaoViatura?.map((manutencao) => (
                                    <Flex key={manutencao.id} overflow={"hidden"} position={"relative"} gap={2} flexDir="column" bgGradient="linear(to-tr, gray.990, gray.990, green.900)" borderTop="1px" shadow={"buttonShadow"} borderColor="green.700" rounded={"md"} p={2}>
                                        <Flex flexDir={"column"}>
                                            ...
                                            <Text color={"gray.500"}>{convertDateAndTime(manutencao.data)}</Text>
                                        </Flex>
                                        <Flex flexDir={"column"}>
                                            <Text zIndex={1}>
                                                Motivo:
                                            </Text>
                                            <Text color={"gray.500"}>{manutencao.motivo}</Text>
                                        </Flex>


                                        <Flex zIndex={0} position={"absolute"} bottom={-10} right={-10} color="green.600" opacity={0.3}>
                                            <VscTools fontSize="150" />
                                        </Flex>
                                    </Flex>
                                ))}
                            </Grid>
