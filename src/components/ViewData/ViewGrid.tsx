    ...
    <Flex flexDirection="column">
      ...
      {data?.length !== 0 ? (
        ...
        >
          {data
            .filter((registro) =>
              finalizados ? registro : registro.status === "ativo"
            )
            .filter((registro) => (registro.civilId ? registro : null))
            .map((registro) => (
              <Flex
                position="relative"
                key={registro.id}
              ...
              >
                {finalizados && registro.status === "finalizado" ? (
                  <Flex
                    ...
                    shadow="buttonShadow"
                    align="center"
                    bgGradient="linear(to-tr, green.900, green.600, green.400)"
                    position="absolute"
                    right={0}
                    top={0}
                  >
                ...
                <Flex flexDirection="column" gap={1}>
                  {registro.status === "finalizado" ? null : (
                    <>
                      <Button
                        onClick={() => handleSubmitForm(registro.id, "entrada")}
                        isDisabled={registro.entrada ? true : false}
                        size="xs"
                        bg="green.600"
                        borderRadius={4}
                        boxShadow="buttonShadow"
                      ...
                      <Button
                        onClick={() => handleSubmitForm(registro.id, "saida")}
                        isDisabled={registro.saida ? true : false}
                        size="xs"
                        bg="orange.600"
                        borderRadius={4}
                        boxShadow="buttonShadow"
                      ...
                      <Button
                        onClick={() =>
                          handleSubmitForm(registro.id, "finalizar")
                        }
                        size="xs"
                        bg="red.600"
                        borderRadius={4}
      ...
      {data.length !== 0 ? (
        ...
        >
          {data
            .filter((registro) => (registro.militarId ? registro : null))
            .filter((registro) =>
              finalizados ? registro : registro.status === "ativo"
            )
            .map((registro) => (
